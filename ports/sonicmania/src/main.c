...
#define SO_NAME "libsonicmania.so"
...
static int try_skip_sentinel_crash(ucontext_t *uc, uintptr_t fault_addr) {
  ...
  if (fault_addr < 0x10000 && (pc < text || pc >= text + text_size)) {
    ...
    extern void *g_cpp_base;
    if (g_cpp_base && pc >= (uintptr_t)g_cpp_base && pc < (uintptr_t)g_cpp_base + 0x400000)
      fprintf(stderr, "[DIAG] libc++ crash off=0x%lx fault=0x%lx\n", (unsigned long)(pc - (uintptr_t)g_cpp_base), (unsigned long)fault_addr);
...
static int g_comb_n;
static void load_libcpp_module(void) {
  size_t sz = 32 * 1024 * 1024;
  void *h = mmap(NULL, sz, PROT_READ | PROT_WRITE | PROT_EXEC,
                 MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
  if (h == MAP_FAILED) fatal_error("libc++ heap fail");
  if (so_load("./libc++_shared.so", h, sz) < 0)
    fatal_error("so_load libc++_shared falhou");
  if (so_relocate() < 0) fatal_error("relocate libc++ falhou");
  so_resolve(dynlib_functions, dynlib_numfunctions, 0);
  so_finalize();
  so_flush_caches();
  so_execute_init_array();
  g_cpp_base = text_base;
  int cxx_n = 0;
  DynLibFunction *cxx = so_snapshot_symbols(&cxx_n);
  fprintf(stderr, "[dl] libc++_shared via so_loader: %d simbolos\n", cxx_n);
  g_comb_n = (int)dynlib_numfunctions + cxx_n;
  g_comb = malloc(sizeof(DynLibFunction) * g_comb_n);
  memcpy(g_comb, dynlib_functions, sizeof(DynLibFunction) * dynlib_numfunctions);
  if (cxx && cxx_n > 0)
    memcpy(g_comb + dynlib_numfunctions, cxx, sizeof(DynLibFunction) * cxx_n);
}
...
int main(int argc, char *argv[]) {
  ...
  // ===== 2-MODULOS (padrao reVC): libc++_shared 1o via so_loader =====
  load_libcpp_module();
  // Load the game .so
  ...
  if (so_resolve(g_comb, g_comb_n, 0) < 0)
  ...
  debugPrintf("=== Sonic Mania JNI driver ===\n");
  jni_run();
